...
class principale(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("GC57 Gestione Messaggi Criptati")
        self.geometry("600x500")
        self.config(bg="#5F9EA0")

        introduzione = """Benvenuto nel sistema di cifratura GC57 QuantumShield.\n\nQuesto programma utilizza un sistema crittografico basato su semiprimi di grandi dimensioni, che vengono fattorizzati istantaneamente tramite l'algoritmo GC57.\n\nLa sicurezza si basa sulla combinazione di questi semiprimi con una chiave derivata da una chiavetta USB.\n\nI dati (messaggi e allegati) vengono cifrati con blocchi la cui dimensione dipende dalla grandezza dei fattori primi estratti, rendendo ogni cifratura unica e resistente a qualsiasi attacco, anche ad attacchi quantistici."""
...
class Invia(tk.Toplevel):
    ...
    def codifica(self):
        ...
        if save_path:
            salva_file_cifrato(
                save_path, cifrato, semiprimo, blocco_byte, self.semiprimo_codice
            )
